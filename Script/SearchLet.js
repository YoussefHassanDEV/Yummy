class Searchlet {
    ...
    async showSearch() {
        try {
            const response = await fetch('https://www.themealdb.com/api/json/v1/1/search.php?f=' + this.fstlet.val());
            if (!response.ok) {
            ...
            $("#searchLetRow").empty()
            // const container = document.querySelector('#searchLetRow').innerHTML;
